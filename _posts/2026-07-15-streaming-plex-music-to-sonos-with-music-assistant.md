...
A note on the hardware: the DS418j is a modest ARM NAS with 512MB of RAM, but since it's only serving music files, with no transcoding involved, it copes fine. The initial library sync on the Pi is the only slow part of the whole setup.
